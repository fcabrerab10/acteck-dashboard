...
export default function BarraGrupos({ arbol, activo, onEntrada, perfil }) {
  ...
  const inverso = theme.surfaceInverse || theme.surfaceDark;
  const sobreInverso = theme.textOnInverse || theme.textOnDark;
  return (
    <nav aria-label="Menú" style={{
      ...
      {entradas.map(({ id, label, icon: Icon }) => {
        const on = activo === id;
        return (
          <button key={id} type="button" onClick={() => onEntrada(id)} aria-current={on ? 'page' : undefined} aria-label={label}
            style={{
              ...
              background: on ? inverso : 'transparent', color: on ? sobreInverso : theme.textMuted,
      ...
      <button type="button" onClick={() => onEntrada('perfil')} aria-label="Perfil y preferencias" aria-current={activo === 'perfil' ? 'page' : undefined}
        style={{ width: 48, height: 44, flexShrink: 0, border: 0, borderRadius: 999, padding: 0, cursor: 'pointer', background: activo === 'perfil' ? inverso : 'transparent', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', transition: `background ${DUR.state}ms ${EASE}` }}>
