...
function AnalisisCliente({ cliente, clienteKey }) {
  ...
  var scorecard = React.useMemo(function() {
    ...
    items.push({ label: "Sell-Through YTD", value: fmtPct(ytd.st), color: stColor, detail: "Meta: >80%" });
    // Cumplimiento cuota
  ...
  var section = function(title, icon, children) {
    return el("div", { style: { background:"#1e293b", borderRadius:12, padding:"16px 20px", marginBottom:12 } },
      el("h3", { style: { margin:"0 0 12px", fontSize:14, fontWeight:700, color:"#f1f5f9", display:"flex", alignItems:"center", gap:8 } }, icon, " ", title),
      children
    );
  };
  ...
  var metricBox = function(label, value, sub, color) {
    return el("div", { style: { background:"#0f172a", borderRadius:10, padding:"12px 16px", flex:1, minWidth:140, borderTop:"3px solid " + (color || "#3b82f6") } },
      el("div", { style: { fontSize:11, color:"#94a3b8", marginBottom:4 } }, label),
      el("div", { style: { fontSize:20, fontWeight:700, color:"#f1f5f9" } }, value),
  ...
  return el("div", { style: { maxWidth:1100, margin:"0 auto", color:"#f1f5f9" } },
    // Header
    el("div", { style: { display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 } },
      ...
      el("select", { value: anio, onChange: function(e) { setAnio(Number(e.target.value)); }, style: { padding:"5px 10px", borderRadius:8, border:"1px solid #334155", background:"#0f172a", color:"#f1f5f9", fontSize:12 } },
    ...
    section("Scorecard", "\uD83D\uDEA6",
      el("div", { style: { display:"flex", gap:10, flexWrap:"wrap" } },
        scorecard.map(function(s, i) {
          return el("div", { key: i, style: { flex:1, minWidth:150, background:"#0f172a", borderRadius:10, padding:"14px 16px", borderLeft:"4px solid " + s.color } },
    ...
    section("Sell-Through Mensual", "\uD83D\uDD04",
      el("div", null,
        ...
        el("div", { style: { display:"flex", gap:10, marginBottom:16, flexWrap:"wrap" } },
          ...
          metricBox("Sell-Through YTD", fmtPct(ytd.st), ytd.st >= 80 ? "Saludable" : ytd.st >= 50 ? "Moderado" : "Bajo", ytd.st >= 80 ? "#10b981" : ytd.st >= 50 ? "#f59e0b" : "#ef4444")
        ),
        // Monthly bars
        ...
        el("div", { style: { marginTop:16 } },
          el("div", { style: { fontSize:12, color:"#94a3b8", marginBottom:8, fontWeight:600 } }, "Sell-Through % por Mes"),
          el("div", { style: { display:"flex", gap:6, flexWrap:"wrap" } },
            ventasPorMes.map(function(v) {
              ...
              return el("div", { key: v.mes, style: { textAlign:"center", width:70, background:"#0f172a", borderRadius:8, padding:"8px 4px", borderBottom:"3px solid " + color } },
      ...
      el("div", null,
        ...
        el("div", { style: { display:"grid", gridTemplateColumns:"repeat(6, 1fr)", gap:6 } },
          ventasPorMes.slice(0, 6).map(function(v) {
            ...
            return el("div", { key: v.mes, style: { background:"#0f172a", borderRadius:8, padding:"10px 8px", textAlign:"center" } },
    ...
    skuAnalysis ? section("M\u00E1rgenes del Canal", "\uD83D\uDCB0",
      el("div", null,
        ...
        Object.keys(skuAnalysis.byMarca).length > 0 ? el("div", { style: { display:"flex", gap:10, marginBottom:16, flexWrap:"wrap" } },
          Object.keys(skuAnalysis.byMarca).map(function(marca) {
            ...
            return el("div", { key: marca, style: { flex:1, minWidth:200, background:"#0f172a", borderRadius:10, padding:"12px 16px" } },
              el("div", { style: { fontSize:13, fontWeight:600, color:"#f1f5f9", marginBottom:6 } }, marca || "Sin marca"),
          ...
          el("table", { style: { width:"100%", fontSize:11, borderCollapse:"collapse" } },
            ...
            el("tbody", null,
              skuAnalysis.all.filter(function(s){return s.margenPesos>0;}).sort(function(a,b){return b.margenPesos-a.margenPesos;}).slice(0,10).map(function(s) {
                return el("tr", { key: s.sku, style: { borderBottom:"1px solid #1e293b" } },
                  el("td", { style: { padding:"6px 8px", color:"#94a3b8", fontFamily:"monospace", fontSize:10 } }, s.sku),
                  el("td", { style: { padding:"6px 8px", color:"#f1f5f9", maxWidth:180, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" } }, s.desc),
        ...
        el("div", null,
          ...
          skuAnalysis.topSO.map(function(s, i) {
            return el("div", { key: s.sku, style: { display:"flex", alignItems:"center", gap:8, padding:"6px 8px", background: i % 2 === 0 ? "#0f172a" : "transparent", borderRadius:6, fontSize:11 } },
              ...
              el("span", { style: { flex:1, color:"#f1f5f9", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" } }, s.desc || s.sku),
      ...
      el("div", null,
        ...
        skuAnalysis.sinVenta90.length > 0 ? el("div", null,
          ...
          skuAnalysis.sinVenta90.slice(0, 8).map(function(s) {
            return el("div", { key: s.sku, style: { display:"flex", alignItems:"center", gap:8, padding:"6px 10px", background:"#0f172a", borderRadius:6, marginBottom:4, fontSize:11 } },
              el("span", { style: { color:"#94a3b8", fontFamily:"monospace", fontSize:10, width:80 } }, s.sku),
              el("span", { style: { flex:1, color:"#f1f5f9" } }, s.desc),
          ...
          el("div", { style: { display:"flex", gap:4, alignItems:"flex-end", height:120 } },
            ventasPorMes.map(function(v) {
              ...
              return el("div", { key: v.mes, style: { flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:2 } },
                ...
                el("div", { style: { width:"100%", height: h + "px", background: isReal ? "#3b82f6" : "#334155", borderRadius:3, border: isReal ? "none" : "1px dashed #475569" } }),
