...
// ─── DATOS ML (se sobreescriben con fetch dinámico) ──────────────────────────
...
const clientes = {
  ...
  mercadolibre: {
    ...
    frecuencia: "Semanal",
    color: "#FFE600",
    cuotaAnual: 0, // Sin cuota definida aún
    kpis: {
      sellInMes: 0,
      cuotaMes: 0,
      cuotaMes25M: 0,
    ...
    tendencia: [],
    pendientes: [
      { id: 1, tarea: "Optimizar listados de productos con bajo CTR", responsable: "Fernando", fecha: "2026-04-15", estado: "pendiente" },
      { id: 2, tarea: "Revisar precios vs competencia en categoría gaming", responsable: "Fernando", fecha: "2026-04-20", estado: "pendiente" },
    ],
    pagos: [],
    promocionesActivas: [],
    minuta: {
...
function calcularSalud(kpis, pagos) {
  if (!kpis.cuotaAcumulada || kpis.cuotaAcumulada === 0) return "verde"; // ML sin cuota
...
function TarjetaSellOutMarca({ sellOutMarca, totalOrdenes }) {
  const marcas = Object.entries(sellOutMarca || {}).sort((a, b) => b[1] - a[1]);
  const total = marcas.reduce((s, [, v]) => s + v, 0);
  const coloresMarca = {
    "ACTECK": { bg: "bg-red-100", text: "text-red-700", bar: "bg-red-500" },
    "BALAM RUSH": { bg: "bg-blue-100", text: "text-blue-700", bar: "bg-blue-500" },
    "OTRO": { bg: "bg-gray-100", text: "text-gray-600", bar: "bg-gray-400" },
  };
  return (
    <div className="bg-white rounded-2xl shadow-sm p-5">
      <CardHeader titulo="Sell Out por Marca (ML)" icono="🏷️" />
      <div className="space-y-3">
        {marcas.map(([marca, monto]) => {
          const pct = total > 0 ? Math.round((monto / total) * 100) : 0;
          const c = coloresMarca[marca] || coloresMarca["OTRO"];
          return (
            <div key={marca}>
              <div className="flex justify-between items-center text-sm mb-1">
                <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${c.bg} ${c.text}`}>{marca}</span>
      ...
      <div className="border-t pt-3 mt-3 flex justify-between items-center">
        <span className="text-sm text-gray-500">Total sell-out ML</span>
        <span className="font-bold text-gray-800">{formatMXN(total)}</span>
      </div>
      {totalOrdenes > 0 && (
        <p className="text-xs text-gray-400 mt-1 text-right">{totalOrdenes.toLocaleString()} órdenes procesadas</p>
      )}
    </div>
  );
}

function TarjetaTendenciaML({ tendencia }) {
  if (!tendencia || tendencia.length === 0) return null;
  const maxVal = Math.max(...tendencia.map(t => t.sellOut));
  return (
    <div className="bg-white rounded-2xl shadow-sm p-5">
      <CardHeader titulo="Tendencia Sell Out ML por Mes" icono="📈" />
      <div className="space-y-3">
        {tendencia.map(t => {
          const pct = maxVal > 0 ? Math.round((t.sellOut / maxVal) * 100) : 0;
          return (
            <div key={t.mes}>
              <div className="flex justify-between items-center text-sm mb-1">
                <span className="font-medium text-gray-700">{t.mes}</span>
                <span className="font-bold text-gray-800">{formatMXN(t.sellOut)}</span>
              </div>
              <div className="h-3 bg-gray-100 rounded-full overflow-hidden flex">
                {t.acteck > 0 && (
                  <div className="h-full bg-red-500" style={{ width: `${(t.acteck / maxVal) * 100}%` }} title={`Acteck: ${formatMXN(t.acteck)}`}></div>
                )}
                {t.balamRush > 0 && (
                  <div className="h-full bg-blue-500" style={{ width: `${(t.balamRush / maxVal) * 100}%` }} title={`Balam Rush: ${formatMXN(t.balamRush)}`}></div>
                )}
                {t.otro > 0 && (
                  <div className="h-full bg-gray-400" style={{ width: `${(t.otro / maxVal) * 100}%` }} title={`Otro: ${formatMXN(t.otro)}`}></div>
                )}
              </div>
              <div className="flex gap-3 text-xs text-gray-400 mt-0.5">
                <span className="text-red-600">Acteck: {formatMXN(t.acteck)}</span>
                <span className="text-blue-600">BR: {formatMXN(t.balamRush)}</span>
                <span>Otro: {formatMXN(t.otro)}</span>
              </div>
            </div>
          );
        })}
      ...
      <div className="flex gap-4 mt-3 pt-3 border-t text-xs">
        <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500"></span> Acteck</span>
        <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-blue-500"></span> Balam Rush</span>
        <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-gray-400"></span> Otro</span>
      </div>
    </div>
  );
}

function HomeCliente({ cliente, isML }) {
  const c = cliente;
  const k = c.kpis;
  ...
  const pctCuotaAnual = c.cuotaAnual > 0 ? Math.round((k.sellInAcumulado / c.cuotaAnual) * 100) : 0;
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl flex items-center justify-center font-bold text-lg"
                 style={{ backgroundColor: c.color, color: c.color === "#FFE600" ? "#333" : "#fff" }}>
              {c.nombre[0]}
            </div>
            <div>
              <h1 className="text-2xl font-bold text-gray-800">{c.nombre}</h1>
              <p className="text-sm text-gray-400">
                <span className="font-medium" style={{ color: c.color === "#FFE600" ? "#D4A017" : c.color }}>{c.marca}</span>
          ...
          <div className="flex items-center gap-3">
            {isML && <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-sm font-semibold bg-yellow-100 text-yellow-700"><span className="w-2 h-2 rounded-full bg-yellow-400"></span> ML en vivo</span>}
            {!isML && <Semaforo estado={salud} />}
            <span className="text-xs text-gray-400">Actualizado: {formatFecha(new Date().toISOString().slice(0,10))}</span>
          </div>
        </div>
      </div>

      {/* ── SECCIÓN ML: KPIs de Sell Out ── */}
      {isML ? (
        <>
          <div className="grid grid-cols-1 gap-4 mb-4 md:grid-cols-3">
            ...
            />
            <KPICard
              label="Total Órdenes ML"
              valor={(c.totalOrdenes || 0).toLocaleString()}
              sub="Órdenes pagadas 2026"
              color="#FFE600"
            />
            <KPICard
              label="Ticket Promedio"
              valor={c.totalOrdenes > 0 ? formatMXN(k.sellOutAcumulado / c.totalOrdenes) : "—"}
              sub="Monto promedio por orden"
              color="#0ea5e9"
            />
          </div>

          <div className="grid grid-cols-1 gap-6 mb-6 md:grid-cols-2">
            <TarjetaSellOutMarca sellOutMarca={c.sellOutMarca} totalOrdenes={c.totalOrdenes} />
            <TarjetaTendenciaML tendencia={c.tendencia} />
          </div>
        </>
      ) : (
        <>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        ...
        {c.pagos.length > 0 && <TarjetaPagos pagos={c.pagos} />}
        {c.promocionesActivas.length > 0 && <TarjetaPromociones promos={c.promocionesActivas} />}
        {c.minuta.acuerdos.length > 0 && <TarjetaMinuta minuta={c.minuta} />}
      </div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    fetch('/api/ml-sellout?year=2026')
      .then(r => r.json())
      .then(data => {
        if (!cancelled && data.sellOutPorMes) {
          setMlData(data);
        }
      })
  ...
  // ── Enriquecer cliente ML con datos reales ──
  const clientesDinamicos = { ...clientes };
  if (mlData) {
    const soMes = mlData.sellOutPorMes || {};
    const soMarca = mlData.sellOutPorMarca || {};
    const soMesMarca = mlData.sellOutPorMesMarca || {};
    const mesesConDatos = Object.keys(soMes).map(Number).sort((a, b) => a - b);
    const ultimoMesML = mesesConDatos.length > 0 ? mesesConDatos[mesesConDatos.length - 1] : 0;
    const sellOutAcumulado = Object.values(soMes).reduce((a, b) => a + b, 0);

    clientesDinamicos.mercadolibre = {
      ...clientes.mercadolibre,
      kpis: {
        ...clientes.mercadolibre.kpis,
        sellOut: soMes[ultimoMesML] || 0,
        sellOutAcumulado,
        ultimoMes: NOMBRES_MES[ultimoMesML] || "—",
      },
      tendencia: mesesConDatos.map(m => ({
        mes: NOMBRES_MES[m]?.slice(0, 3) || m,
        sellOut: soMes[m] || 0,
        acteck: soMesMarca[m]?.['ACTECK'] || 0,
        balamRush: soMesMarca[m]?.['BALAM RUSH'] || 0,
        otro: soMesMarca[m]?.['OTRO'] || 0,
      })),
      sellOutMarca: soMarca,
      totalOrdenes: mlData.totalOrdenes,
    };
  }

  const c = clientesDinamicos[clienteActivo];
  ...
  return (
    <div className="flex h-screen bg-gray-50 font-sans">
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        <HomeCliente cliente={c} isML={clienteActivo === "mercadolibre"} />
      </main>
