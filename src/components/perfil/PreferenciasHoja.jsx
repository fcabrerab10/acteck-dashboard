...
import { Camera, Trash2, User, Briefcase, Mail, KeyRound, Sun, Moon, Palette, Monitor, Rows3, Rows4, Sparkles, LayoutGrid, Star, ArrowUp, ArrowDown, X, Bell, Keyboard, Info, LogOut, Shield, ChevronRight, Smartphone, CalendarCheck } from 'lucide-react';
import { useTheme } from '../../lib/themeContext';
import { TYPO } from '../../lib/themeTokens';
import { supabase, DB_CONFIGURED } from '../../lib/supabase';
import { usePreferencias, setPreferencia, getPath, MODOS_MENU, MODOS_MENU_MOVIL, MODOS_AGENDA } from '../../lib/preferencias';
    ...
    <>
      <HojaLateral abierto={abierto} onClose={onClose} theme={theme} titulo="Preferencias" sub="Los cambios se guardan al instante.">
        ...
        <Grupo theme={theme}>
          ...
          <Fila theme={theme} icon={CalendarCheck} label="Agenda" sub={MODOS_AGENDA.find((m) => m.id === (prefs?.agenda?.modo === 'tablero' ? 'tablero' : 'bandeja'))?.desc}>
            <Segmented value={prefs?.agenda?.modo === 'tablero' ? 'tablero' : 'bandeja'} onChange={(v) => setPreferencia('agenda.modo', v)} options={MODOS_AGENDA.map((m) => ({ id: m.id, label: m.label, title: m.desc }))} />
          </Fila>
