...
export default function SellInDicotech({ clienteKey }) {
  ...
  // Copilot recos · sin SO no incluimos la reco "SKUs sin rotación"
  const copilotRecos = useMemo(() => {
    ...
    if (yoyDisplay != null) {
      const tone = yoyDisplay >= 0 ? 'info' : 'warn';
      const emoji = yoyDisplay >= 0 ? 'Crece' : 'Cae';
      const suf = yoyEsProyectado ? ' (proy. cierre)' : '';
      arr.push({ sev: tone, title: `${MESES_LARGO[mesActual - 1]} ${emoji} ${Math.abs(yoyDisplay).toFixed(1)}% YoY${suf}`, sub: `${fmt.money(mesActualData.monto)} vs ${fmt.money(mesActualData.prevMonto)} en ${anioPrev}` });
    }
    return arr.slice(0, 3);
  }, [pctMTD, mesActualData, familiasYTD, yoyDisplay, yoyEsProyectado, mesActual, anioPrev]);
  ...
  return (
    <div style={{ fontFamily: TYPO.fontText, color: theme.text, display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.35fr) minmax(0, 1fr)', gap: 10 }}>
        <TimelineLineal theme={theme} P={P} data={timelineMeses} sums={timelineSums} rango={rango} onChangeRango={setRango} anio={anio} anioPrev={anioPrev} mesActual={mesActual} cuotaSimple={cuotaSimple} />
...
// ═══════════════ Timeline Lineal ═══════════════
function TimelineLineal({ theme, P, data, sums, rango, onChangeRango, anio, anioPrev, mesActual, cuotaSimple }) {
  ...
  return (
    <div style={{ background: theme.surface, border: `1px solid ${theme.border}`, borderRadius: 12, padding: '14px 16px' }}>
      ...
      <div style={{ display: 'flex', gap: 12, padding: '6px 0 8px', flexWrap: 'wrap', borderBottom: `1px solid ${theme.divider || theme.border}`, marginBottom: 6 }}>
        ...
        <SumStat theme={theme} k={<><Dot color={P.orange} dashed />{cuotaSimple ? 'Cuota' : 'Cuota ideal'}</>} v={fmt.money(sums.cuota)} vColor={theme.text} />
        {sums.deltaYoY != null && (
      ...
      <div style={{ position: 'relative' }}>
        ...
        {hovered && !hovered.futuro && (
          <TimelineTooltip theme={theme} P={P} data={hovered} anio={anio} anioPrev={anioPrev} cuotaSimple={cuotaSimple}
...
function TimelineTooltip({ theme, P, data, anio, anioPrev, cuotaSimple, xPct }) {
  ...
  return (
    <div style={{
      ...
      {data.cuota > 0 && (
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10.5, marginTop: 2 }}>
          <span style={{ color: theme.textMuted }}>{cuotaSimple ? 'Cuota' : 'Cuota ideal'}</span>
...
function FerruMini({ size = 14 }) {
  return (
    <svg width={size} height={size * 1.07} viewBox="0 0 140 150">
      <defs>
        <radialGradient id="fSellInDicoMini" cx="35%" cy="30%">
          <stop offset="0%" stopColor="#F5E6FF" />
          <stop offset="100%" stopColor="#AF52DE" />
        </radialGradient>
      </defs>
      <path d="M 25 40 Q 25 15 70 15 Q 115 15 115 40 L 115 100 Q 115 105 110 105 Q 105 100 100 105 Q 95 110 90 105 Q 85 100 80 105 Q 75 110 70 105 Q 65 100 60 105 Q 55 110 50 105 Q 45 100 40 105 Q 35 110 30 105 Q 25 100 25 95 Z" fill="url(#fSellInDicoMini)" />
...
function FerruStars() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', pointerEvents: 'none' }}>
      <style>{`@keyframes fSellInDicoTwinkle { 0%,100% { opacity:0.35; transform:scale(0.8); } 50% { opacity:1; transform:scale(1.3); } }`}</style>
      {stars.map((s, i) => (
        <span key={i} style={{
          position: 'absolute', top: s.top, left: s.left,
          width: 2, height: 2, borderRadius: 999, background: '#FFF',
          boxShadow: '0 0 6px rgba(255,255,255,0.7)',
          animation: `fSellInDicoTwinkle 3s ease-in-out ${s.d}s infinite`,
        }} />
      ))}
    </div>
  );
