// HomePcel V2 · rediseño Apple con timeline lineal y Ferruteck cósmico
...
export default function HomePcel({ cliente, clienteKey }) {
  const perfil = usePerfil();
  if (!puedeVerPestanaCliente(perfil, clienteKey || 'pcel', 'home')) {
    return <SinAcceso motivo={`No tienes acceso al Resumen de ${clienteKey || 'pcel'}.`} />;
  }
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const soSint = (soMenR.data || []).map((r) => ({
        fecha: `${r.anio}-${String(r.mes).padStart(2, '0')}-15`,
        total: Number(r.monto) || 0,
        cantidad: Number(r.piezas) || 0,
        no_parte: null,
        marca: null,
      }));
      setSellOutDetalle(soSint);
  ...
  const marcasSIvsSO = useMemo(() => {
    ...
    // Sell In agregado por marca: facturacion_clientes joined con roadmap_sku por sku
    const skuMap = {};
    roadmapPcel.forEach(p => { skuMap[String(p.sku)] = p; });
    const siByMarca = {};
    facturacion.forEach(r => {
      if (Number(r.anio) !== anio) return;
    ...
    // Sell Out agregado por marca: v_sellout_pcel_marca_mes (ya viene agregado)
    const soByMarca = {};
    sellOutMarcaMesPcel.forEach(r => {
      if (Number(r.anio) !== anio) return;
      if (!meses.includes(Number(r.mes))) return;
  ...
  }, [facturacion, sellOutMarcaMesPcel, roadmapPcel, marcaRango, anio]);
