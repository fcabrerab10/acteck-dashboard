            ...
            <tbody>
                      ...
                      <td className="py-1 px-1.5 text-right whitespace-nowrap" style={{ background: '#EEF2FF' }}>
                        {r.invStock > 0 ? (
                          <>
                            ...
                            {r.invValor > 0 && <div className="text-[10px] text-indigo-500 tabular-nums">{formatMXN(r.invValor)}</div>}
                          </>
                        ) : (
              ...
              <tr className="font-semibold text-gray-800 bg-gray-50" style={{ borderTop: '2px solid #E5E7EB' }}>
                ...
                <td className="py-1.5 px-1.5 text-right whitespace-nowrap" style={{ background: '#E0E7FF' }}>
                  ...
                  {invTotales.valor > 0 && <div className="text-[10px] text-indigo-600 tabular-nums">{formatMXN(invTotales.valor)}</div>}
                </td>
              </tr>
            </tbody>
          </table>
...
function BloqueSucursales({ sucursales, matriz, mesActual, anioActual, anioPrev, inventarioSucursales = [], meta }) {
  ...
  // Si la fuente del cliente no trae valor de inventario, el bloque se lee en piezas.
  const hayInvValor = useMemo(() => inventarioSucursales.some((iv) => (Number(iv.valor) || 0) > 0), [inventarioSucursales]);
  ...
  const valorMetrica = (it) => {
    ...
    if (metrica === 'inventario') return hayInvValor ? it.invValor : it.invStock;
    return 0;
  };
  const fmtMetrica = (v) => {
    if (metrica === 'monto' || (metrica === 'inventario' && hayInvValor)) return formatMXN(v);
    return fmtInt(v);
  };
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-4">
      ...
      <div className="flex justify-between items-start gap-4 mb-3">
        ...
        <div className="inline-flex bg-gray-100 rounded-lg p-0.5 gap-0.5">
          {[
            ...
            { k: 'piezas', l: 'Piezas' },
            { k: 'tx', l: 'Tickets' },
            { k: 'inventario', l: hayInvValor ? 'Inventario $' : 'Inventario pz' },
          ].map((m) => (
      ...
      <BentoSucursales
        ...
        totalVis={totalVis} selKey={selKey} setSelKey={setSelKey} hayInvValor={hayInvValor} />
        ...
        <div className="mt-4 p-4 bg-sky-50 border border-sky-200 rounded-xl">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-3">
            ...
            <div className="bg-white border border-sky-200 rounded-md p-2.5">
              ...
              <div className="text-[10px] text-gray-500 tabular-nums">
                {seleccionada.invValor > 0 ? formatMXN(seleccionada.invValor) : `${fmtInt(seleccionada.invStock)} pz en piso`}
      ...
      <div className="mt-4 pt-4 border-t border-gray-200 grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div className="bg-gray-50 rounded-md p-3">
          ...
          <div className="text-[10.5px] text-gray-500 tabular-nums">
            {totalValFisicas > 0 ? `${formatMXN(totalValFisicas)} · ` : ''}{fisicas.filter((x) => x.invStock > 0).length} sucursales
          </div>
        </div>
      </div>
    </div>
...
function BentoSucursales({ fisicas, virtuales, matriz, mesActual, metrica, valorMetrica, fmtMetrica, totalVis, selKey, setSelKey, hayInvValor = true }) {
      ...
      <button onClick={() => setSelKey(isSel ? null : s.key)}
        ...
        {s.invStock > 0 && (
          <div className="text-[10px] text-indigo-800 bg-indigo-50 px-1.5 py-0.5 rounded font-semibold tabular-nums self-start">
            📦 {fmtInt(s.invStock)} pz{hayInvValor && s.invValor > 0 ? ` · ${formatMXN(s.invValor)}` : ''}
          </div>
        )}
      </button>
    );
...
function AnalisisPcelSku({ sku, rows, sellInAcumulado, anioActual, mesActual, accent, siPzYTD, clienteNombre }) {
  ...
  const [invActeck, setInvActeck] = useState(null);
  const [cargando, setCargando] = useState(true);
  const [hoverIdx, setHoverIdx] = useState(null);
    ...
    (async () => {
      try {
        ...
        const [semRes, proxRes, ultRes, invRes] = await Promise.all([
          ...
          // Próximo arribo pendiente (activo + futuro)
          supabase.from('embarques_compras')
            .select('arribo_cedis,shp_qty,po,cedis,estatus')
            .eq('codigo', sku)
            ...
            .limit(1)
            .maybeSingle(),
          // Último arribo concluido (para mostrar contexto si no hay próximo)
          supabase.from('embarques_compras')
            .select('arribo_cedis,shp_qty,po,cedis')
            .eq('codigo', sku)
            .eq('estatus', 'CONCLUIDO')
            .not('arribo_cedis', 'is', null)
            .order('arribo_cedis', { ascending: false })
            .limit(1)
            .maybeSingle(),
          // Inventario disponible en almacenes Acteck
          supabase.from('v_inventario_comercial')
            .select('disponible,inventario,costo_promedio,costo_disponible,almacenes_con_stock')
            .eq('sku', sku)
            .maybeSingle(),
        ]);
        if (cancelled) return;
        setSemanal(semRes.error ? [] : (semRes.data || []));
        setProximoArribo(proxRes.error ? null : (proxRes.data || null));
        setUltimoArribo(ultRes.error ? null : (ultRes.data || null));
        setInvActeck(invRes.error ? null : (invRes.data || null));
        setCargando(false);
      } catch (e) {
        if (cancelled) return;
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="bg-gray-50 rounded-md p-2.5 border border-gray-100">
              ...
              {kpis.costoActual > 0 && <div className="text-[10px] text-gray-500 tabular-nums">{formatMXN(kpis.stock * kpis.costoActual)} a costo</div>}
            </div>
