...
- **Proyectos y abasto (3.25.0 · 2026-09-21)**: la pestaña `forecastReservas` (antes "Forecast · Reservas") se rehízo desde cero como **Proyectos y abasto** (`src/modules/comercial/ProyectosAbasto.jsx` + `proyectos/`; móvil `src/movil/pestanas/Proyectos.jsx`). Un proyecto = venta comprometida (cliente, mes, SKUs) en las tablas nuevas `proyectos` / `proyecto_lineas` + vista `v_proyectos_sku_mes` (migración `20260921_proyectos_abasto.sql`, RLS como `sugeridos_compra`, trigger `fn_auditoria`).
  - Tres vistas: **Tablero** (columna por mes, tarjetas arrastrables + panel "Qué falta comprar"), **Matriz SKU × mes** (celda coloreada por cobertura; selector Necesidad · Faltante · Reservado) e **Historial** (`auditoria_cambios`).
  - Motor puro en `proyectos/calculo.js` (16 pruebas en `scripts/test-proyectos-calculo.mjs`): reparto **FIFO por mes** de disponible + tránsito con ETA ≤ fin de mes; fecha límite de compra = mes objetivo − lead time real (`v_lead_time_sku`, respaldo por proveedor, default 104 d); los % se recalculan sobre totales, nunca se promedian.
  - "Mandar al S&OP" **reusa** `solicitudes_compra` / `_lineas` (borrador abierto o nuevo); Excel por `src/lib/exportar.js`. Alertas nuevas `proyecto_sin_cobertura` y `arribo_tarde_proyecto` (área forecast) en `api/cron.js` con el mismo motor.
  - `ForecastReservas.jsx` y `reservas/` quedan en el repo **sin uso**. Guía: `docs/PROYECTOS_ABASTO.md`.
- **Proyectos y forecast (3.45.0 · 2026-09-25)**: la pestaña `forecastReservas` se renombró («Proyectos y abasto» → «Proyectos y forecast»; ids y permiso `forecast_reservas` no cambian). Objetivo de Fernando: saber, mes a mes y por cliente, cuánto dinero hay que cerrar. Migración `20260925_proyectos_precio.sql`: `proyecto_lineas.precio` y `.lista` («__custom» si es personalizado). `useAbasto` trae `precios` (sku → { lista: precio } de `v_estrategia_precios_lista`); en `HojaProyecto` al elegir el SKU se propone la lista natural del cliente (`LISTA_POR_CLIENTE` de propuestas) con `PrecioPicker` (listas + personalizado) y `CampoNumero` para piezas; columnas SKU · Pz · Precio · Monto · Disp. hoy · Llega (fecha y piezas del próximo embarque) · Falta, y total en dinero. `calculo.js`: `porProyecto.monto/montoCubierto/sinPrecio`, `tablero()` suma `monto`, `montoPorCliente` y `montoPorProb` (propuesto · probable · confirmado; entregado y cancelado no suman), `resumen.monto/montoMesActual/montoConfirmado`; `PROBABILIDADES` etiqueta «Propuesto». KPI «Monto comprometido», columnas del tablero con el total y chips por cliente, tarjeta con monto. Avisos en `api/cron.js#reglasArribosProyecto` (generar-alertas 07:00 CDMX): `arribo_proximo_proyecto` cuando hoy + 3 días hábiles = ETA (`sumarDiasHabiles`/`diasHabilesEntre`) y `arribo_hoy_proyecto` el día del ETA; sin `para_usuario` (los ven Fernando y Karolina en la campana y el resumen de las 09:00). Pruebas: `scripts/test-proyectos-calculo.mjs` (22).
- **Arranque sin destello e ícono de la app (3.45.1 · 2026-09-25)**: el tema vive en `perfiles.tema_ui` y llegaba después del primer paint (pantalla blanca → oscura). `applyThemeToRoot` guarda `{ key, bg, text }` en `localStorage.acteck_tema_v1` y un `<script>` en línea en `index.html` lo aplica antes de React (`data-theme`, `--t-bg`, `--t-text`, `color-scheme`, `theme-color`); sin nada guardado sigue a `prefers-color-scheme`. Ícono instalado (iOS/Android): wordmark «acteck.» blanco con punto azul (#0A84FF) sobre negro, generado desde SVG con `qlmanage -t` (macOS) en `public/apple-touch-icon.png` (180) y `public/pwa/icon-{192,512,1024,maskable-512}.png`; `favicon.svg` = «a.» sobre negro; manifiesto `name: 'Acteck'`, `background_color: '#000000'`. Para regenerar: SVG 1024 con `-apple-system`, `qlmanage -t -s 1024 -o dir icon.svg`, `sips -z`.
- **Responsive por dispositivo (2026-09-21)**: `src/lib/dispositivo.js` es la única fuente del "modo" de pantalla
  (`telefono` < 700 · `tabletaCompacta` 700-899 y 900-1099 táctil vertical · `tableta` 900-1279 táctil ·
  `laptop` 900-1279 sin táctil y 1280-1899 · `panoramico` ≥ 1900) y de las preferencias **por máquina**
...
- **Propuestas · armador (3.40.1–3.41.0 · 2026-09-24)**: Fernando armando una propuesta real pidió: quitar líneas (× por línea y «Vaciar propuesta» en `MiPropuesta.jsx`), editar piezas en «Mi propuesta» (−/+ de 5 y captura) y **antes** de aceptar (columna Piezas del catálogo es un campo en todas las filas, `piezasPrevias` en `Armar.jsx`; Enter agrega; la pastilla «Sugerido · 35 pz → 50» avisa que va con su cantidad), **lista de precios para toda la propuesta** (select en la barra de filtros: aplica a las líneas que la tengan y queda de default para las nuevas; cada línea puede seguir cambiándola), y **Revisar como pantalla completa** (ya no `HojaLateral`; botón «Armador» para volver) con descripción completa + familia · marca · roadmap, Inv cli, ⌀ 3m, Inv Ack (rojo si pides más de lo que hay) y SPIFF por línea. `ExportMenu` pinta su menú con `createPortal` en `body` y posición fija (el `Panel` con `overflow: hidden` lo recortaba). `input.prop-piezas` en `index.css` quita las flechas del navegador.
- **Propuestas · sugeridos con inventario y arribos (3.42.0 · 2026-09-24)**: `sugeridos.js#sugeridoDe` ya topa las piezas al **disponible en Acteck** (`dispActeck` = Σ `inventario_acteck.disponible` en almacenes comerciales; en múltiplos de 5). Si no alcanza ni para 5 el SKU sale como «Sin stock Acteck · llega 21 oct (600 pz)» (pill roja, sin Aceptar, fuera de «Aceptar todos»); si alcanza parcial, «Sugerido · 5 pz de 55» en naranja. `datos.js#fetchCatalogo` trae `v_transito_sku` y arma `arribo = { fecha, piezas, po, estatus, total }` (embarque con ETA más cercana ≥ hoy); columna **Llega** en armador y Revisar. Captura numérica: `CampoNumero.jsx` (texto con `inputMode`, estado local mientras se edita, miles al salir, Enter confirma) sustituye a todos los `type="number"` de piezas y precio personalizado (Armar, MiPropuesta, Revisar, PrecioPicker). Pruebas: `scripts/test-propuestas-sugeridos.mjs`.
- **Propuestas · 3.42.1–3.42.4 (2026-09-24)**: columna **Últ. compra** (último mes con piezas facturadas al cliente en `facturacion_clientes`, «Abr 26 · 200 pz»; la última propuesta enviada pasó al tooltip) en armador y Revisar. Ordenar por cualquier columna respeta el orden: los sugeridos sólo se fijan arriba con el orden por defecto (`sellout90`); el sombreado azul cubre a todos los sugeridos. **Costo** para el margen viene de `v_medidas_inventario_sku.costo_promedio` (se pedía `precios_sku.costo_promedio`, columna inexistente: todo salía «sin costo»). **Buscador tolerante** (`filtros.js#normalizarBusqueda`/`tokens`/`coincide`, índice con el SKU sin guion): sin acentos, signos como espacio, punto sólo entre dígitos; pruebas `scripts/test-propuestas-busqueda.mjs`.
...
- **Tracking · «Nuevo pedido» (3.44.0 · 2026-09-24)**: acuerdo con Fernando: dos formas de cargar un pedido. (1) **Desde la OC del cliente** = captura manual (`FormOC`, formulario o «pegar correo»; cada cliente manda la OC distinto: PDF, Excel…). (2) **Desde una factura del ERP** (`tracking/NuevoPedido.jsx`): lista de facturas sin pedido (`facturasSinOC`, 30 días), se eligen una o varias del mismo cliente y el pedido nace con sus productos, piezas y precio (`guardarOC` con `fuente:'factura'`, número = referencia de la factura o, sin ella, el folio); al crearlo se abre en la tabla para registrar envíos. Un pedido acepta varias facturas y varios envíos parciales (no cambió). Los botones «Registrar OC» y «Pegar correo» se fundieron en «Nuevo pedido»; el panel «Facturas sin OC» ahora dice «Facturas del ERP sin pedido» con «Crear pedido». El móvil de Tracking sigue legacy.
- **Agenda (2026-09-11 · base)**: pestaña `agenda` debajo de Inicio (permiso global `agenda`; `adminInterna` redirige). Modelo `agenda_items` (tareas y puntos de reunión; `cliente_key` #cliente, `responsables` @persona; estados abierta·hecha·cancelada·arrastrada) + `agenda_reuniones` (reuniones con minuta, eventos y —desde V4— viajes) + `agenda_google`. Migraciones `20260911_agenda_modelo.sql` (RLS `agenda_puede_ver/editar`, auditoría, `alertas.para_usuario`, copia idempotente de `pendientes_equipo`/`pendientes`/`minutas`/`minuta_acuerdos`/`eventos_equipo`; RPCs `agenda_cerrar_reunion` y `agenda_arrastrar_pendientes`) y `20260911_agenda_google.sql`. Google Calendar en `api/google-calendar.js`. Las tablas viejas no se borran; `PendientesCalendarioV2` en `src/_archivo/agenda-v2/`.
- **Agenda V4 (2026-09-21) · `docs/AGENDA_V4.md`**: rediseño pedido por Fernando («no me funciona»: las alertas de SKUs tapaban los pendientes). **En la Agenda sólo hay `agenda_items` y cuentas; las alertas del sistema se quedan en la campana.** Arriba, calendario de ancho completo (`Calendario.jsx`, Mes/Semana, clic en un día → Pendiente/Reunión/Viaje, arrastrar cambia la fecha); debajo Segmented **Pendientes · Reuniones · Cuentas · Archivados**: captura en una línea + bloques Vencidos·Hoy·Esta semana·Más adelante·Sin fecha, minutas, cuentas que Fernando sigue y lo cerrado con «Desarchivar».
- **Agenda V4 · modelo** (`supabase/migrations/20260921_agenda_v4.sql`): `agenda_subtareas` (checklist "3/10"; **el padre nunca se cierra solo**, sólo se sugiere), `cuentas_seguimiento` + `cuentas_seguimiento_notas` (sembradas con los 10 contactos de la convención CVA + 1 pendiente por cuenta al 25 sep), `agenda_reuniones.tipo` acepta `'viaje'` (ausencias de varios días). **`agenda_puede_ver()` ya no vale por ser interno**: pide super admin o `permisos.globales.agenda ∈ (ver,edit)`; David Millán quedó en `oculto` (ni UI ni PostgREST ni correos — `CORREOS_SIN_AGENDA` en `etiquetas.js` y `conAgenda()` en `api/cron.js`).
- **Agenda V4 · cron**: task `agenda-correo` con dos envíos al día y sólo a Fernando y Karolina — mañana «Lo que dejaste» (vencidos, hoy, asignados desde ayer, cuentas vencidas) y tarde «Lo que tienes mañana» (reuniones, pendientes, viajes, cargas); no manda nada si no hay nada. En `vercel.json` van **dos** entradas (`15,30 14 * * 1-5` = 08:15/08:30 y `0 21,23 * * 1-5` = 15:00/17:00 CDMX) y la task elige por la hora (`destinatariosAhora`); `agenda-hoy` se retiró del cron. Regla nueva `cuenta_seguimiento` en `generar-alertas`.
- **Agenda V4 · código**: `src/modules/agenda/` (Agenda · Calendario · Mes · Semana · Pendientes · Cuentas · Archivados · Subtareas; lógica pura en `calculo.js`, datos en `datos.js` con `useAgendaV4`), móvil en `src/movil/pestanas/agenda/` (Pendientes · Calendario · Reuniones · Cuentas · Archivados, swipe hecho/posponer). Bandeja/Tablero/Equipo y sus gemelos móviles en `src/_archivo/agenda-v3*/`; la preferencia `agenda.modo` se retiró. El Resumen de cada cliente trae el Panel «Últimas minutas y acuerdos» (`MinutasCliente`, navega con `acteck:navegar {pagina:'agenda', extra:{reunionId}}`) y el bloque "Hoy" de Inicio agrupa los avisos de SKUs en una línea por área. Pruebas: `scripts/test-agenda-calculo.mjs` (19 casos), `scripts/test-agenda-ssr.mjs` (13).
- **Agenda V4 · seguimiento por punto (2026-09-21, `supabase/migrations/20260921_agenda_comentarios.sql`)**: debajo de cada punto de la minuta hay un hilo de comentarios (`agenda_item_comentarios`: tipo seguimiento·mejora·acuerdo, `reunion_id` = la reunión en la que se comentó, RLS de `agenda_puede_ver/editar` + auditoría). `calculo.js#hiloComentarios` sigue la cadena de arrastres (`arrastrado_desde` / `origen.item_anterior`) para que el hilo no se pierda entre reuniones; `HojaItem` y la hoja del ítem móvil lo muestran completo en «Seguimiento». Piezas: `Comentarios.jsx` (web y móvil) y `ReunionAnterior.jsx`.
- **Agenda V4 · reunión anterior y orden del día**: la minuta abre plegado el panel «Reunión anterior» (puntos, estado y sus comentarios), con «Traer puntos abiertos» (RPC `agenda_traer_puntos(reunion, desde)`, deja `origen.item_anterior` → «viene de la reunión del 11 ago») y «Ver todas las reuniones de este cliente»; los puntos se capturan antes y se reordenan con ▲▼ (`datos.js#moverPunto`). Un punto con `origen.enlace = {pagina, clienteKey}` ofrece «Ver en dashboard». Sembrada la reunión Digitalife del 22-sep-2026 con sus 12 puntos y 2 proyectos de abasto (bocinas y gabinetes Batauro).
- **Deslizador de vidrio (2026-09-22)**: Preferencias › Apariencia → "Vidrio en barras y hojas" con 3 paradas — **Opaco (default, idéntico a hoy)** · Tintado (tema al 60 % + blur 10) · Vidrio (8 % blanco en oscuro / 55 % en claro, blur 16 + saturate 120 %, hairline luminoso y luz interna, como `Resaltado`). Se guarda **por dispositivo** (`setPrefDispositivo(modo,'vidrio',…)`, DEFAULTS en `src/lib/dispositivo.js`).
  Un solo hook `useVidrio()` (`src/lib/vidrio.js`, montado en `App.jsx`) escribe `--vidrio-bg|blur|border|shine` (+ `-oscuro` para la barra apple.com, que nunca se aclara) en `<html>`; en Opaco **no define nada** y cada pieza resuelve el fallback = su literal de siempre. `prefers-reduced-transparency` / `prefers-reduced-motion` fuerzan Opaco.
  Sólo el **chrome** lo lee (Topbar, BarraApple, BarraIphone, `nav/comun.vidrio()` → SidebarIpad · BarraSuperior móvil · Paleta ⌘K · Hoja iPhone, `perfil/comun.vidrio()` → Modal y HojaLateral, BarraGrupos, Cajón, HojaM). El contenido (Hero, KpiCard, Panel, tablas) no se toca. Pruebas: `node --test scripts/test-vidrio-ssr.mjs`.
- **Minuta legible y por correo (3.38.2–3.39.1 · 2026-09-22)**: Fernando abrió la reunión Digitalife y «no entendía cómo funciona»: el sugeridor de `@persona` (`CampoEtiquetas`) arrancaba abierto y, como cada punto termina en «@fernando», se pintaba debajo de todos; ahora sólo abre con foco. La fila de punto (`Minuta.jsx#LineaPunto`) tiene dos modos: en reposo se LEE (título limpio, responsable, fecha como pastilla que abre el selector, categoría) y al hacer clic en el texto se edita con `#cliente @persona /categoría`; el hilo de comentarios va plegado detrás del globo con contador (abierto si ya trae seguimiento) y «quedó:» aparece al resolver o al abrir el hilo.
  **Enviar por correo al cliente**: botón «Correo» en la fila de acciones (web) y en el pie (celular) → `EnviarMinuta.jsx` / `movil/pestanas/agenda/EnviarMinutaM.jsx`: contactos del cliente en `agenda_contactos` (se recuerdan; alta rápida «Nombre <correo>»), copia siempre a quien envía, mensaje libre y vista previa. Manda `api/google-calendar.js?action=enviar-minuta` (JWT; super admin o `permisos.globales.agenda = 'edit'`; SMTP de Vercel, reply-to del remitente; el armado del correo vive en `api/_minuta.js` con `scripts/test-minuta-correo.mjs`) y anota `agenda_reuniones.envios` (pastilla «Enviada hace…»). Migración `20260922_agenda_envio_minuta.sql`. El modal se pinta con `createPortal` en `body`: un `fixed` dentro de la `HojaLateral` (que lleva `transform`) queda encerrado en la hoja.
- **Cuotas manuales protegidas (2026-09-22, migración `20260922_cuotas_manuales.sql`)**: `cuotas_mensuales.manual` + `manual_nota` y trigger `cuotas_proteger_manual` (BEFORE UPDATE/DELETE): una fila `manual = true` sobrevive al replace por año del puente (DELETE ignorado) y a su upsert (UPDATE ignorado si cambian cifras sin mandar `manual = false` o una `manual_nota` distinta). Así se cargó **Digitalife 2026** desde la tabla "Objetivo Anual 2026" de Fernando: `cuota_min` = escenario 25M · `cuota_ideal` = 30M (Σ 25,000,000 / 30,000,000). Para cambiarla: nueva migración o Configuración con `manual_nota` nueva. Los demás clientes siguen viniendo de BP por el puente.
- **Buscar o preguntar (3.40.0 · 2026-09-24)**: la paleta ⌘K (`nav/Paleta.jsx` + `nav/Respuesta.jsx`) y la pestaña Buscar del celular (`movil/pestanas/Buscar.jsx` + `buscar/RespuestaM.jsx`) aceptan preguntas en lenguaje natural y contestan con la cifra ahí mismo, el botón «Abrir …» y enlaces «También». Módulo `src/lib/preguntas/`: `interpretar.js` (reglas puras, sin red: cliente por alias, SKU `XX-999999`, mes/año/«mes pasado»/«2025»; tipos cuota · ventas · sellout(+foco inventario) · stock · transito · pendientes · reunion · pagos · cobranza · margen · inventarioEmpresa; pruebas `scripts/test-preguntas.mjs`) y `responder.js` (mismas vistas del dashboard: `v_fact_cliente_mes`, `cuotas_mensuales`, `v_facturacion_global_mensual`, `v_medidas_cuota_mes`, `v_sellout_cuenta_mes`, `v_medidas_inventario_sku`, `v_transito_sku`, agenda, `pagos`, `estados_cuenta`, `v_medidas_ventas_cliente_mes`, `v_medidas_inventario`). Permisos: cliente con `puedeVerCliente`, empresa y márgenes con `puedeVerSensible`, agenda/pagos/inventario con su permiso global. **Nada se manda a un servicio externo.** Chips de ejemplo (`SUGERENCIAS`) con la paleta vacía; la respuesta es el ítem 0 (↵ la abre). El gesto «deslizar desde el centro» del celular NO se hizo: chocaba con jalar-para-refrescar; la lupa de la barra abre Buscar.
- **Agenda = pestaña raíz del celular (2026-09-22)**: `TABS_RAIZ = ['inicio','agenda','clientes','alertas','buscar']`; `destino({pagina:'agenda'})` devuelve `{tipo:'tab',tab:'agenda',extra}` (ya no un push) y MovilApp pasa el `extra` de la notificación como `inicial`. Entrada "Agenda" (CalendarCheck) entre Inicio y Comercial en `movil/menu/BarraGrupos.jsx`, sólo con `puedeVerPaginaGlobal(perfil,'agenda')` (David Millán y los externos no la ven), con badge de vencidos + de hoy (`useContadorAgenda`, lee la cache sin consultar). El nodo Agenda sigue en el grupo Inicio del menú: navegar ahí selecciona la pestaña. Pruebas: `node --test scripts/test-movil-barra-ssr.mjs`.
