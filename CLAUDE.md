...
- Pendientes V3: archivar `src/components/Mobile*.jsx`, Administración (diseño A, en curso) (usa la navegación iPhone), pantallas restantes al kit (Estado de Resultados, Configuración, Análisis, Estrategia, Forecast, S&OP, Propuestas), colocar `FrescuraPill` en cada pantalla, borrar `UserMenu`/`Sidebar.jsx` legacy, migrar helpers de formato a `src/lib/format.js`.
