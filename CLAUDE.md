...
- **Arranque sin destello e ícono de la app (3.45.1 · 2026-09-25)**: el tema vive en `perfiles.tema_ui` y llegaba después del primer paint (pantalla blanca → oscura). `applyThemeToRoot` guarda `{ key, bg, text }` en `localStorage.acteck_tema_v1` y un `<script>` en línea en `index.html` lo aplica antes de React (`data-theme`, `--t-bg`, `--t-text`, `color-scheme`, `theme-color`); sin nada guardado sigue a `prefers-color-scheme`. Ícono instalado (iOS/Android): wordmark «acteck.» blanco con punto rojo sobre negro, generado desde SVG con `qlmanage -t` (macOS) en `public/apple-touch-icon.png` (180) y `public/pwa/icon-{192,512,1024,maskable-512}.png`; `favicon.svg` = «a.» sobre negro; manifiesto `name: 'Acteck'`, `background_color: '#000000'`. Para regenerar: SVG 1024 con `-apple-system`, `qlmanage -t -s 1024 -o dir icon.svg`, `sips -z`.
