...
- **`v_vision_margen_canal` lee la tabla vieja `ventas_erp`**, que no se carga desde julio de 2026. Cualquier margen por canal que salga de ahí está congelado. Reapuntar a `v_medidas_ventas_canal_mes`.
...
- **Cuota Piezas y Cuota Costo**: añadir `UNIDADES` y `COSTODEVENTA` a `CUOTAS_COLS` del puente (Mac mini).
